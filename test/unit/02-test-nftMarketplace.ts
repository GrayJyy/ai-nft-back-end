...
import { AiNft, NftMarketplace } from '../../typechain-types'
...
!developChain.includes(network.name)
  ...
  : describe('NftMarketplace', () => {
      ...
      beforeEach(async () => {
        ...
        defaultSigner = await ethers.getSigner(deployer)
        ...
        aiNft = await ethers.getContractAt('AiNft', aiNftDeployment.address, defaultSigner)
        nftAddress = aiNft.address
        nftMarketplaceAddress = nftMarketplaceDeployment.address
        await aiNft.mintNft('', { value: COST })
        await aiNft.approve(nftMarketplaceAddress, TOKEN_ID)
      })
      ...
      describe('listItem', () => {
        ...
        it('Should reverts when do not get approved', async () => {
          await aiNft.approve(ethers.constants.AddressZero, TOKEN_ID)
        ...
        it('Should reverts when not the owner', async () => {
          ...
          console.log(defaultAccount) // 旧的签名者，也是nft的拥有者（因为aiNft调用mint时的签名者就是defaultAccount
          console.log(await aiNft.ownerOf(0))
          assert.equal(await aiNft.ownerOf(0), defaultAccount, 'Not the owner')
          /**
          ...
          await expect(nftMarketplace.listItem(aiNft.address, TOKEN_ID, PRICE)).to.be.revertedWithCustomError(
            nftMarketplace,
            'NftMarketplace__IsNotOwner'
          )
        })
        it('Should updates listing with seller and price', async function () {
          await nftMarketplace.listItem(aiNft.address, TOKEN_ID, PRICE)
          const listing = await nftMarketplace.getListing(aiNft.address, TOKEN_ID)
